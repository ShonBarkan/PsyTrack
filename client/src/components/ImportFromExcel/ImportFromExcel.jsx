...
const ImportFromExcel = () => {
  const { setEnglishTestsScore, setMathTestsScore, setHebrewTestsScore, subject, setTestsScore, setMistakes } = useAppContext();
  ...
  const handleFileUpload = (event) => {
    ...
    reader.onload = (e) => {
      ...
      // Process 'טעויות' sheet
      const mistakeSheet = workbook.Sheets['טעויות'];
      if (mistakeSheet) {
        const jsonData = XLSX.utils.sheet_to_json(mistakeSheet);

        // Reconstruct the original nested structure
        const reconstructedData = jsonData.reduce((acc, row) => {
          if (!acc[row.Key]) acc[row.Key] = {};
          if (!acc[row.Key][row.SubKey]) acc[row.Key][row.SubKey] = [];
          
          // Split the values back into an array
          acc[row.Key][row.SubKey] = row.Values.split(', ').map((value) => value.trim());
          return acc;
        }, {});

        setMistakes(reconstructedData); // Set the reconstructed data
      }
    };
  ...
  return (
    <button className="container-btn-file">
      {omerSVG}
      <span>Upload Scores File</span>
      <input className="file" name="text" type="file" onChange={handleFileUpload} accept=".xlsx, .xls" />
    </button>
  );
};
